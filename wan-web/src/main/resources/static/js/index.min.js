;jQuery( function() {
    ...
    $('#iframe-content').multitabs({
        ...
        cache : config('multitabs_cache') == '1' ? true : false
    });
...
var setSidebar = function(data) {
    ...
    $('.nav-drawer').append(html);
}

var createMenu = function (data, is_frist) {
    var menu_body = is_frist ? '' : '<ul class="nav nav-subnav">';
    
    for(var i = 0; i < data.length; i++){
        icon_div = data[i].pid == null ? '<i class="' + data[i].icon + '"></i>' : '';
        menuName = data[i].pid == null ? '<span>' + data[i].name + '</span>' : data[i].name;
        if (data[i].submenus && data[i].submenus.length > 0) {
            ...
            menu_body += createMenu(data[i].submenus);
        } else {
            menu_body += '<li class="nav-item"><a href="' + data[i].url + '" class="multitabs">' + icon_div + menuName + '</a>';
        }
        menu_body += '</li>';
    }
    
    menu_body += is_frist ? '' : '</ul>';
    return menu_body;
};